...
- comme il est peu probable que la personne qui visionnera votre email ait les fonts "Marianne" sur son ordinateur ou téléphone, il est indispensable de pointer vers des fichiers de fonts distantes (idéalement servies par votre application ou hébergées sur un CDN comme expliqué dans [les prérequis](./?path=/docs/dsfr-connect-prérequis--docs))
