...
Pour tous excepté les ajustements graphiques, cela revient à dépendre de fichiers tiers à fournir pour qu'ils puissent être chargés par le navigateur. Plus communément ils sont distribués par l'application ou sont hébergés sur un CDN.
...
Nous vous conseillons de l'ajuster "aux couleurs" de votre administration afin de respecter la charte graphique de l'État. Le plus simple est sûrement d'utiliser le DSFR dans un mini fichier HTML afin de faire le rendu du logo, et de prendre un screenshot et de retravailler la zone d'affichage (cela vous évitera des prises de tête sur un logiciel d'édition).

Ensuite en fonction du thème utilisé il faudra probablement mettre votre image dans un dossier `assets` comme pour les fonts, et ajuster votre thème pour pointer vers ce nouveau fichier.
